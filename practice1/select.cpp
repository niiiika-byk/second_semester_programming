...
void crossJoin(TableJson& tableJS, const string& table1, const string& table2, const string& column1, const string& column2) {
    ...
    for (size_t iCsv1 = 1; iCsv1 < amountCsv1; iCsv1++) {
        ...
        for (size_t i = 0; i < amountRow1; ++i) { // проходимся по строкам 1 таблицы
            ...
            for (size_t iCsv2 = 1; iCsv2 < amountCsv2; iCsv2++) {
                ...
                for (size_t j = 0; j < amountRow2; ++j) { // проходимся по строкам 2 таблицы
    ...
    if (s != "") {
        ...
        for (size_t iCsv = 1; iCsv < amountCsv; iCsv++) {
            ...
            for (size_t i = 0; i < amountRow; ++i) { // проходимся по строкам
                if (doc.GetCell<string>(columnIndex, i) == s) { // извлекаем значение
                    return true;
                }
            }
        }
...
//выборка
void select(const string& command, TableJson& tableJS) {
    ...
    iss3 >> word;
    if (word != "WHERE") { // если нет условий, то объединяем таблицы
        crossJoin(tableJS, table1, table2, column1, column2);
        return;
    }

    iss3 >> word; // таблица1 и колонка1
    string t1, c1;
